...
exports.TaskEventManager = void 0;
...
class TaskEventManager {
    ...
    taskCreated(task) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot create event for task without ID');
        }
     ...
     * ```
     */
    taskUpdated(task, previousState) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot update event for task without ID');
        }
        this.eventQueue.publish({
            type: 'taskUpdated',
            task,
    ...
    taskCompleted(task) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot complete event for task without ID');
        }
        this.eventQueue.publish({
            type: 'taskCompleted',
            task,
    ...
    taskFailed(task, error) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot fail event for task without ID');
        }
        ...
        this.eventQueue.publish({
            type: 'taskFailed',
            task: Object.assign(Object.assign({}, task), { error: normalizedError }),
            timestamp: Date.now()
        });
    }
    /**
    ...
    taskCanceled(task) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot cancel event for task without ID');
        }
        this.eventQueue.publish({
            type: 'taskCanceled',
            task,
     ...
     * ```
     */
    artifactAdded(task, artifact) {
        if (!task.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot add artifact to task without ID');
        }
        if (!artifact.id) {
            throw new a2a_core_1.InvalidTaskStateError('Cannot add artifact without ID');
        }
        this.eventQueue.publish({
            type: 'artifactAdded',
            task,
...
exports.TaskEventManager = TaskEventManager;
//# sourceMappingURL=task-event-manager.js.map